...
class Theta:
  ...
  def __init__(self,
    ...
    # Shutter speed values by name
    self.__shutter_speed_names = {v: k for k, v in \
					self._shutter_speeds.items()}
  ...
  def set_exposure_program(self,
    ...
    # Sanity-check the arguments and convert the program from a string to a
    # program number
  ...
  def get_shutter_speed(self,
    ...
    Return the name of the shutter speed if found, the shutter speed value
    otherwise
    """

    n = self._get_option("shutterSpeed",
				connect_timeout = connect_timeout,
				reconnect_tries = reconnect_tries,
				read_timeout = read_timeout)

    return self._shutter_speeds.get(n, n)
  ...
  def set_shutter_speed(self,
			...
			reconnect_tries = _default_reconnect_tries,
			read_timeout = _default_request_timeout):
    """Get the current shutter speed by name - one of "auto", "1/25000",
    "1/20000", "1/16000", "1/12800", "1/12500", "1/10000", "1/8000", "1/6400",
    "1/5000", "1/4000", "1/3200", "1/2500", "1/2000", "1/1600", "1/1250",
    "1/1000", "1/800", "1/640", "1/500", "1/400", "1/320", "1/250", "1/200",
    "1/160", "1/125", "1/100", "1/80", "1/60", "1/50", "1/40", "1/30", "1/25",
    "1/20", "1/15", "1/13", "1/10", "1/8", "1/6", "1/5", "1/4", "1/3", "1/2.5",
    "1/2", "1/1.6", "1/1.3", "1", "1.3", "1.6", "2", "2.5", "3.2", "4", "5",
    "6", "8", "10", "13", "15", "20", "25", "30", "40", "50", "60"
    If an int or float value is passed instead, use it as-is instead
    """

    # Sanity-check the arguments and convert the shutter speed from a string to
    # a program number
    if isinstance(speed, str):
      assert speed in self.__shutter_speed_names, \
		"Shutter speed should be {}".\
			format(" or ".
				join(", ".join('"{}"'.format(f) \
					for f in self.__shutter_speed_names
					).rsplit(", ", 1)))
      speed = self.__shutter_speed_names[speed]

    assert type(speed) in (int, float), \
		"speed should be an int, a float or a string"

    return self._set_option("shutterSpeed", speed,
				connect_timeout = connect_timeout,
				reconnect_tries = reconnect_tries,
				read_timeout = read_timeout)
